(() => {
  ...
  const STORAGE_KEY = "snkr_catalog_section_v82";
...
;(() => {
  ...
  const DRAG = 'snkr-v81-gallery-dragging';
  const ANIM = 'snkr-v81-gallery-animating';
  const MOVED = 'snkr-v81-gallery-moved';
...
;(() => {
  // v80: catalog filter modal. Background is locked; the modal is always fully inside the viewport.
  ...
  function apply() {
    ...
    // Keep the React-owned filter layer inside #root so React onClick handlers keep working.
    layer.classList.toggle('snkr-v82-open', open);
  ...
  function closeByNativeFallback(event) {
    const target = event.target;
    const closeTarget = target?.closest?.('.sheet-close,.sheet-handle,.filter-backdrop');
    if (!closeTarget || !isOpen()) return;
    // Do not stop the event: React's own onClick must close the state.
    // This fallback only makes sure the page unlocks right after React closes the sheet.
  ...
  document.addEventListener('click', closeByNativeFallback, { capture: true, passive: true });
  new MutationObserver((mutations) => {

(() => {
  ...
  function closeGuaranteeScreen(){
    document.querySelector('.snkr-v109-guarantee-screen')?.remove();
    document.querySelectorAll('.snkr-v109-hidden-screen').forEach((el) => {
      el.classList.remove('snkr-v109-hidden-screen');
      el.removeAttribute('aria-hidden');
    });
    document.body.classList.remove('snkr-v109-guarantee-open');
  }
  ...
  function ensureGuaranteeProfileTab(){
    ...
    if (!list || list.querySelector('[data-snkr-open-guarantee="true"]')) return;

    const row = document.createElement('button');
    row.type = 'button';
    row.className = 'service-list-row snkr-v109-guarantee-tab';
  ...
  function makeGuaranteeScreen(){
    ...
    section.className = 'screen detail-screen snkr-v109-guarantee-screen';
    section.setAttribute('aria-labelledby', 'snkr-v109-guarantee-title');
    section.innerHTML = `
      <button class="back-row snkr-v109-back" type="button" aria-label="Назад в профиль">
        ${ICONS.back}<span>Назад</span>
      </button>
      <p class="detail-kicker">SERVICE</p>
      <h1 class="detail-title snkr-v109-title" id="snkr-v109-guarantee-title">Гарантия</h1>

      <div class="snkr-v109-card snkr-v109-hero">
        <span class="snkr-v109-icon">${ICONS.shield}</span>
        <div class="snkr-v109-hero-copy">
      ...
      <div class="snkr-v109-card snkr-v109-copy">
      ...
      <div class="snkr-v109-grid">
        <div class="snkr-v109-card snkr-v109-mini">
          <span class="snkr-v109-icon snkr-v109-icon--small">${ICONS.rotate}</span>
          <h3>14 дней</h3>
          <p>На возврат или обмен пары, если товар не подошёл по параметрам.</p>
        </div>
        <div class="snkr-v109-card snkr-v109-mini">
          <span class="snkr-v109-icon snkr-v109-icon--small">${ICONS.defect}</span>
          <h3>30 дней</h3>
          <p>Гарантия в случае брака или производственного дефекта.</p>
        </div>
      </div>

      <div class="snkr-v109-card snkr-v109-note">
        <span class="snkr-v109-icon snkr-v109-icon--row">${ICONS.note}</span>
        <div class="snkr-v109-note-copy">
          <h3>Важно</h3>
      ...
      <button class="snkr-v109-card snkr-v109-manager" type="button" aria-label="Связаться с менеджером">
        <span class="snkr-v109-icon snkr-v109-icon--row">${ICONS.manager}</span>
        <span class="snkr-v109-manager-label">Связаться с менеджером</span>
      </button>
    `;
    return section;
  }
  ...
  function openGuaranteeScreen(){
    ...
    Array.from(slot.children).forEach((child) => {
      if (child.nodeType === 1) {
        child.classList.add('snkr-v109-hidden-screen');
        child.setAttribute('aria-hidden', 'true');
      }
    });
    slot.appendChild(makeGuaranteeScreen());
    document.body.classList.add('snkr-v109-guarantee-open');
  ...
  document.addEventListener('click', (event) => {
    ...
    if (event.target?.closest?.('.snkr-v109-back')) {
      event.preventDefault();
      event.stopPropagation();
      closeGuaranteeScreen();
      return;
    }
    if (event.target?.closest?.('.snkr-v109-manager')) {
      event.preventDefault();
      event.stopPropagation();
      openManager();
      return;
  ...
  window.SNKR_GUARANTEE_V109 = { version: VERSION, openGuaranteeScreen, closeGuaranteeScreen, patchPaymentScreen };
})();

(() => {
  ...
  const STORAGE_KEY = "snkr_catalog_section_v80";
  ...
  if (typeof window.fetch === "function" && !window.__snkrSideSectionsFetchPatchedV80) {
    window.__snkrSideSectionsFetchPatchedV80 = true;
  ...
  function interceptSideMenuFilterClicks() {
    if (document.__snkrSideMenuFilterClicksV80) return;
    document.__snkrSideMenuFilterClicksV80 = true;
  ...
  function interceptBottomCatalogReset() {
    if (document.__snkrBottomCatalogResetV80) return;
    document.__snkrBottomCatalogResetV80 = true;
    document.addEventListener("click", (event) => {
...
;(() => {
  // v80: very smooth product gallery. One gesture = one photo. Vertical gesture = normal page scroll.
  const TRACK = '.product-detail-gallery .product-gallery-track';
  const SLIDE = '.product-gallery-slide';
  const DOT = '.product-gallery-dot';
  const DRAG = 'snkr-v80-gallery-dragging';
  const ANIM = 'snkr-v80-gallery-animating';
  const MOVED = 'snkr-v80-gallery-moved';
...
;(() => {
  ...
  function apply() {
    ...
    layer.classList.toggle('snkr-v80-open', open);

(() => {
  ...
  const ICONS = {
    socials: '<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.55" stroke-linecap="round" stroke-linejoin="round"><path d="M8.5 10.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Z"/><path d="M18 9a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z"/><path d="M3.5 20.5c.45-3.35 2.5-5.3 5-5.3s4.55 1.95 5 5.3"/><path d="M14.2 15.4c.8-.9 1.85-1.4 3.05-1.4 2 0 3.6 1.55 4 4.3"/></svg>',
  ...
  function closeSocialScreen() {
    const screen = document.querySelector('.snkr-v86-social-screen');
    if (screen) screen.remove();
    document.querySelectorAll('.snkr-v86-hidden-screen').forEach((el) => {
      el.classList.remove('snkr-v86-hidden-screen');
      el.removeAttribute('aria-hidden');
    });
    document.body.classList.remove('snkr-v86-social-open');
  }
  ...
  function enhanceHomeInfo() {
    document.querySelectorAll('.info-row-item').forEach((item) => {
      ...
      if (!target) return;
      item.classList.add('snkr-v86-home-link');
  ...
  function ensureSocialProfileTab() {
    ...
    profile.querySelectorAll('.snkr-v85-socials').forEach((el) => el.remove());

    if (list.querySelector('[data-snkr-open-socials="true"]')) return;
    const row = document.createElement('button');
    row.type = 'button';
    row.className = 'service-list-row snkr-v86-social-tab';
  ...
  function makeSocialScreen() {
    ...
    section.className = 'screen detail-screen snkr-v86-social-screen';
    section.setAttribute('aria-labelledby', 'snkr-v86-social-title');
    section.innerHTML = `
      <button class="back-row snkr-v86-social-back" type="button" aria-label="Назад в профиль">
        ${ICONS.back}<span>Назад</span>
      </button>
      <p class="detail-kicker">SNKR LAND</p>
      <h1 class="detail-title snkr-v86-social-title" id="snkr-v86-social-title">Наши соц. сети</h1>
      <div class="about-hero-card snkr-v86-social-hero">
      ...
      <div class="snkr-v86-social-list" aria-label="Социальные сети Sneakers Land">
        ${SOCIALS.map((item) => `
          <button class="detail-card detail-card-row snkr-v86-social-link snkr-v86-social-link-${item.id}" type="button" data-snkr-social="${item.id}" aria-label="Открыть ${item.label}">
            <span class="snkr-v86-social-link-icon">${ICONS[item.icon]}</span>
            <span class="snkr-v86-social-link-copy">
              <strong>${item.label}</strong>
              <small>${item.note}</small>
            </span>
            <span class="snkr-v86-social-link-arrow">${ICONS.arrow}</span>
          </button>
        `).join('')}
      </div>
    `;
  ...
  function openSocialScreen() {
    ...
    Array.from(slot.children).forEach((child) => {
      if (child.nodeType === 1) {
        child.classList.add('snkr-v86-hidden-screen');
        child.setAttribute('aria-hidden', 'true');
      }
    });
    slot.appendChild(makeSocialScreen());
    document.body.classList.add('snkr-v86-social-open');
    window.scrollTo({ top: 0, left: 0, behavior: 'auto' });
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    const back = event.target?.closest?.('.snkr-v86-social-back');
    if (back) {
      event.preventDefault();
      event.stopPropagation();
      closeSocialScreen();
  ...
  window.SNKR_PROFILE_SOCIALS_V86 = { version: VERSION, socials: SOCIALS, openSocialScreen, closeSocialScreen };
})();

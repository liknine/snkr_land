...
;(() => {
  ...
  function animateTo(track, targetIndex, duration = 460) {
    if (!track) return;
    cancelAnim(track);
    ...
    const ms = clamp(duration, 360, 560);
    const frame = (now) => {
  ...
  window.__snkrSmoothGallerySnap = (track, targetIndex) => animateTo(track, targetIndex, 460);

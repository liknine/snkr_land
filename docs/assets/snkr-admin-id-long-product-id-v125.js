(() => {
  ...
  // Known admin Telegram IDs found/used in the project.
  // If a new admin is added later, add their Telegram ID here or into window.SNKR_CONFIG.ADMIN_IDS.
  ...
  function isAdmin() {
    ...
    if (params.get('admin') === '1' || window.location.hash.includes('admin')) return true;
  ...
  function productImage(product) {
    ...
    return toStr(image).split('?')[0].split('/').pop();
  }
  ...
  function getDisplayProductId(product, productIndex) {
    // The bot admin panel shows the product as "Товар #<id>".
    // That is the long technical product.id from products.json, so display it as-is.
    const rawId = toStr(product?.id);
  ...
  function ensureBadge(container, isDetail) {
    let badge = container.querySelector(':scope > .snkr-admin-product-id, .snkr-admin-product-id');
  ...
  function placeBadge(container, product, productIndex, isDetail = false) {
    ...
    const badge = ensureBadge(container, isDetail);
    badge.textContent = `Товар #${displayId}`;
  ...
  window.SNKR_ADMIN_ID_LONG_PRODUCT_ID_V125 = { version: VERSION, schedule, adminIds: getAdminIds() };
  // Backward-compatible debug hook.
  window.SNKR_ADMIN_ID_AND_DETAIL_PHOTO_V125 = window.SNKR_ADMIN_ID_LONG_PRODUCT_ID_V125;
})();

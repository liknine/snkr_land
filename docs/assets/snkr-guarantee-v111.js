(() => {
  ...
  function closeGuaranteeScreen(){
    document.querySelector('.snkr-v111-guarantee-screen')?.remove();
    document.querySelectorAll('.snkr-v111-hidden-screen').forEach((el) => {
      el.classList.remove('snkr-v111-hidden-screen');
      el.removeAttribute('aria-hidden');
    });
    document.body.classList.remove('snkr-v111-guarantee-open');
  }
  ...
  function ensureGuaranteeProfileTab(){
    ...
    row.type = 'button';
    row.className = 'service-list-row snkr-v111-guarantee-tab';
  ...
  function makeGuaranteeScreen(){
    ...
    section.className = 'screen detail-screen snkr-v111-guarantee-screen';
    section.setAttribute('aria-labelledby', 'snkr-v111-guarantee-title');
    section.innerHTML = `
      <button class="back-row snkr-v111-back" type="button" aria-label="Назад в профиль">
        ${ICONS.back}<span>Назад</span>
      </button>
      <p class="detail-kicker">SERVICE</p>
      <h1 class="detail-title snkr-v111-title" id="snkr-v111-guarantee-title">Гарантия</h1>

      <div class="snkr-v111-card snkr-v111-hero">
        <span class="snkr-v111-icon">${ICONS.shield}</span>
        <div class="snkr-v111-hero-copy">
      ...
      <div class="snkr-v111-card snkr-v111-copy">
      ...
      <div class="snkr-v111-grid">
        <div class="snkr-v111-card snkr-v111-mini snkr-v111-mini--return">
          <span class="snkr-v111-icon snkr-v111-icon--small">${ICONS.rotate}</span>
          <h3>14 дней</h3>
          <p>На возврат или обмен пары, если товар не подошёл по параметрам.</p>
        </div>
        <div class="snkr-v111-card snkr-v111-mini snkr-v111-mini--warranty">
          <span class="snkr-v111-icon snkr-v111-icon--small">${ICONS.defect}</span>
          <h3>30 дней</h3>
          <p>Гарантия в случае брака или производственного дефекта.</p>
        </div>
      </div>

      <div class="snkr-v111-card snkr-v111-note">
        <span class="snkr-v111-icon snkr-v111-icon--row">${ICONS.note}</span>
        <div class="snkr-v111-note-copy">
          <h3>Важно</h3>
      ...
      <button class="snkr-v111-card snkr-v111-manager" type="button" aria-label="Связаться с менеджером">
        <span class="snkr-v111-icon snkr-v111-icon--row">${ICONS.manager}</span>
        <span class="snkr-v111-manager-label">Связаться с менеджером</span>
      </button>
    `;
    return section;
  }
  ...
  function openGuaranteeScreen(){
    ...
    Array.from(slot.children).forEach((child) => {
      if (child.nodeType === 1) {
        child.classList.add('snkr-v111-hidden-screen');
        child.setAttribute('aria-hidden', 'true');
      }
    });
    slot.appendChild(makeGuaranteeScreen());
    document.body.classList.add('snkr-v111-guarantee-open');
  ...
  document.addEventListener('click', (event) => {
    ...
    if (event.target?.closest?.('.snkr-v111-back')) {
      event.preventDefault();
      event.stopPropagation();
      closeGuaranteeScreen();
      return;
    }
    if (event.target?.closest?.('.snkr-v111-manager')) {
      event.preventDefault();
      event.stopPropagation();
      openManager();
      return;
  ...
  window.SNKR_GUARANTEE_V111 = { version: VERSION, openGuaranteeScreen, closeGuaranteeScreen, patchPaymentScreen };
})();

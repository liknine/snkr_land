(() => {
  ...
  const STORAGE_KEY = "snkr_catalog_section_v78";
  ...
  if (typeof window.fetch === "function" && !window.__snkrSideSectionsFetchPatchedV78) {
    window.__snkrSideSectionsFetchPatchedV78 = true;
  ...
  function interceptSideMenuFilterClicks() {
    if (document.__snkrSideMenuFilterClicksV78) return;
    document.__snkrSideMenuFilterClicksV78 = true;
  ...
  function interceptBottomCatalogReset() {
    if (document.__snkrBottomCatalogResetV78) return;
    document.__snkrBottomCatalogResetV78 = true;
    document.addEventListener("click", (event) => {
...
;(() => {
  // v78: stable product gallery. One swipe = one photo. Vertical movement = normal page scroll.
  const TRACK = '.product-detail-gallery .product-gallery-track';
  const SLIDE = '.product-gallery-slide';
  const DOT = '.product-gallery-dot';
  const DRAG = 'snkr-v78-gallery-dragging';
  const ANIM = 'snkr-v78-gallery-animating';
  ...
  function animateTo(track, targetIndex, duration = 430) {
    if (!track) return;
    cancelAnim(track);
    ...
    if (Math.abs(to - from) < 0.75) {
      ...
      track.classList.remove(DRAG, ANIM);
      return;
    }
    track.classList.add(ANIM);
    const start = performance.now();
    const ms = clamp(duration, 320, 520);
    const frame = (now) => {
  ...
  window.__snkrSmoothGallerySnap = (track, targetIndex) => animateTo(track, targetIndex, 430);
  ...
  function clearLegacyLocks() {
    const classes = [
      'snkr-gallery-native-touch','snkr-gallery-swiping','snkr-gallery-v69-horizontal','snkr-gallery-v75-horizontal','snkr-gallery-v76-horizontal','snkr-gallery-v77-horizontal'
    ];
    document.documentElement.classList.remove(...classes);
    document.body.classList.remove(...classes);
    document.querySelectorAll('.product-gallery-track.is-dragging,.product-gallery-track.snkr-v69-dragging,.product-gallery-track.snkr-v75-dragging,.product-gallery-track.snkr-v76-dragging,.product-gallery-track.snkr-v77-gallery-dragging,.product-detail-gallery.is-swiping,.product-detail-gallery.snkr-v69-swiping')
      .forEach((node) => node.classList.remove('is-dragging','snkr-v69-dragging','snkr-v75-dragging','snkr-v76-dragging','snkr-v77-gallery-dragging','is-swiping','snkr-v69-swiping'));
  }
  ...
  function start(event) {
    ...
    // Own this gesture so old gallery handlers do not fight with it.
    event.stopPropagation();
  ...
  function move(event) {
    ...
    if (state.mode === 'pending') {
      if (ax < 6 && ay < 6) return;
      if (ay > ax * 1.08) {
        // Vertical = page scroll. Do not prevent default.
        state.mode = 'vertical';
        cleanup();
        return;
      }
    ...
    let next = state.startIndex * w - dx;
    if (next < minLeft) next = minLeft + (next - minLeft) * 0.28;
    if (next > maxLeft) next = maxLeft + (next - maxLeft) * 0.28;
  ...
  function end(event) {
    ...
    if (state.mode !== 'horizontal' || ay > ax * 1.22) {
    ...
    const threshold = Math.max(44, width(track) * 0.16);
    let target = state.startIndex;
    if (dx <= -threshold || state.velocity < -0.55) target = state.startIndex + 1;
    else if (dx >= threshold || state.velocity > 0.55) target = state.startIndex - 1;
    target = clamp(target, 0, maxIndex(track));
    cleanup();
    animateTo(track, target, 430);
  }
...
;(() => {
  // v78: catalog filter sheet. Hidden means no blur; opened means centered-lower and visible.
  ...
  function apply() {
    ...
    layer.classList.toggle('snkr-v78-open', open);
    if (!open) return;

    const viewportH = Math.max(520, window.innerHeight || document.documentElement.clientHeight || 720);
    // Lower than v76/v77, but still high enough so buttons remain reachable.
    const top = Math.round(clamp(viewportH * 0.18, 104, 148));
    setVar(layer, '--snkr-filter-top', `${top}px`);
    setVar(layer, '--snkr-filter-max-height', `${Math.max(390, viewportH - top - 16)}px`);
  }

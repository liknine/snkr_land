(() => {
  ...
  const STORAGE_KEY = "snkr_catalog_section_v76";
  ...
  if (typeof window.fetch === "function" && !window.__snkrSideSectionsFetchPatchedV76) {
    window.__snkrSideSectionsFetchPatchedV76 = true;
  ...
  function interceptSideMenuFilterClicks() {
    if (document.__snkrSideMenuFilterClicksV76) return;
    document.__snkrSideMenuFilterClicksV76 = true;
  ...
  function interceptBottomCatalogReset() {
    if (document.__snkrBottomCatalogResetV76) return;
    document.__snkrBottomCatalogResetV76 = true;
    document.addEventListener("click", (event) => {
...
;(() => {
  // v76: controlled one-photo gallery. One swipe = max one photo, vertical scroll stays native.
  const TRACK = '.product-gallery-track';
  const SLIDE = '.product-gallery-slide';
  const DOT = '.product-gallery-dot';
  const READY = 'snkr-v76-gallery-ready';
  const DRAG = 'snkr-v76-dragging';
  const LOCK = 'snkr-gallery-v76-horizontal';
  ...
  function getWidth(node = track) {
    const host = node?.closest?.('.product-detail-gallery') || gallery;
    return Math.max(1, Math.round(node?.clientWidth || host?.clientWidth || 1));
  }
  ...
  function getIndex(node = track) {
    return clamp(Math.round((node?.scrollLeft || 0) / getWidth(node)), 0, maxIndex(node));
  }
  ...
  function clearOldGalleryLocks() {
    ...
    document.querySelectorAll('.product-gallery-track.is-dragging, .product-gallery-track.snkr-v69-dragging, .product-gallery-track.snkr-v75-dragging, .product-detail-gallery.is-swiping, .product-detail-gallery.snkr-v69-swiping')
      .forEach((node) => node.classList.remove('is-dragging', 'snkr-v69-dragging', 'snkr-v75-dragging', 'is-swiping', 'snkr-v69-swiping'));
  }
  ...
  function snapTo(index, node = track, host = gallery, duration = 330) {
    if (!node) return;
    cancelAnim();
    ...
    const ms = clamp(duration + distance * 0.04, 260, 390);
    const started = performance.now();
    const frame = (now) => {
      const p = Math.min(1, (now - started) / ms);
      node.scrollLeft = from + (to - from) * easeOutCubic(p);
      syncDots(host, clamp(Math.round(node.scrollLeft / w), 0, max));
  ...
  function cleanupSoon(now = false) {
    window.clearTimeout(cleanTimer);
    const done = () => {
      track?.classList.remove(DRAG);
      document.documentElement.classList.remove(LOCK, 'snkr-gallery-native-touch', 'snkr-gallery-swiping', 'snkr-gallery-v69-horizontal', 'snkr-gallery-v75-horizontal');
      document.body.classList.remove(LOCK, 'snkr-gallery-native-touch', 'snkr-gallery-swiping', 'snkr-gallery-v69-horizontal', 'snkr-gallery-v75-horizontal');
      active = false;
      horizontal = false;
      track = null;
      gallery = null;
    ...
    if (now) done();
    else cleanTimer = window.setTimeout(done, 410);
  }

  function onStart(event) {
    const node = event.target?.closest?.(TRACK);
    const host = node?.closest?.('.product-detail-gallery');
    if (!node || !host || slides(node).length <= 1) return;
    if (event.target?.closest?.('.product-gallery-dots, button, a')) return;

    // Own the gesture before old gallery handlers; default vertical page scroll still works.
    event.stopPropagation();
    ...
    velocity = 0;
    startIndex = getIndex(node);
    node.scrollLeft = startIndex * getWidth(node);
    syncDots(host, startIndex);
    active = true;
    horizontal = false;
  }
  ...
  function onMove(event) {
    ...
    if (!horizontal) {
      ...
      if (absX > 8 && absX > absY * 1.06) beginHorizontal();
    }

    if (!horizontal) return;
    if (event.cancelable) event.preventDefault();

    const w = getWidth(track);
    const max = maxIndex(track);
    const baseLeft = startIndex * w;
    const maxDrag = w * 0.72; // visual resistance: even a very fast swipe cannot run through several slides.
    const dragged = clamp(-dx, -maxDrag, maxDrag);
    let next = baseLeft + dragged;
    if (next < 0) next *= 0.28;
    const edge = max * w;
    if (next > edge) next = edge + (next - edge) * 0.28;
    track.scrollLeft = next;
    syncDots(gallery, clamp(Math.round(next / w), 0, max));
  }
  ...
  function onEnd(event) {
    ...
    const dx = p.x - startX;
    const absX = Math.abs(dx);
    const absY = Math.abs(p.y - startY);

    event.stopPropagation();
    event.stopImmediatePropagation?.();

    if (!horizontal || absY > absX * 1.12) {
      snapTo(startIndex, node, host, 250);
      cleanupSoon(true);
      return;
    }

    if (event.cancelable) event.preventDefault();
    const threshold = Math.max(38, getWidth(node) * 0.16);
  ...
  function onDotClick(event) {
    ...
    if (index >= 0) snapTo(index, node, host, 300);
  }

  function normalize(node) {
    if (!node || node.classList.contains(READY)) return;
    node.classList.add(READY);
    node.scrollLeft = getIndex(node) * getWidth(node);
    syncDots(node.closest('.product-detail-gallery'), getIndex(node));
  }

  function init(root = document) {
    root.querySelectorAll?.(TRACK).forEach(normalize);
  }
  ...
  document.addEventListener('scroll', (event) => {
    ...
    requestAnimationFrame(() => syncDots(node.closest('.product-detail-gallery'), getIndex(node)));
...
;(() => {
  // v76: raise the catalog filter sheet inside the viewport without moving React DOM nodes.
  const LAYER = '.filter-sheet-layer';
  const OPEN_CLASS = 'snkr-filters-open';

  function normalizeFilterLayer() {
    ...
    layer.style.setProperty('--snkr-filter-top-shift', `${shift}px`);
    layer.style.setProperty('--snkr-filter-max-height', `${Math.max(360, viewportH - desiredTop * 2)}px`);
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', normalizeFilterLayer, { once: true });
  else normalizeFilterLayer();

  window.addEventListener('resize', normalizeFilterLayer, { passive: true });
  window.addEventListener('orientationchange', () => setTimeout(normalizeFilterLayer, 80), { passive: true });

  new MutationObserver(normalizeFilterLayer).observe(document.documentElement, {
    childList: true,
    subtree: true,
    attributes: true,

...
function sameIdentity(order: any, telegramId: number | null, username: string, localClientOrderIds: Set<string>) {
  const orderTelegramId = String(order.telegramId ?? order.telegram_id ?? "");
  const orderUsername = normalizeUsername(String(order.username ?? ""));
  const orderClientOrderId = String(order.clientOrderId ?? order.client_order_id ?? "");
  const safeUsername = username && username !== "dev_user" ? username : "";

  // Static orders.json is public data, so never show every order just because
  // Telegram Desktop did not expose initDataUnsafe.user. In that case, only
  // show orders created on this device/session by matching client_order_id.
  if (telegramId && orderTelegramId && orderTelegramId === String(telegramId)) return true;
  if (safeUsername && orderUsername && orderUsername === safeUsername) return true;
...
export async function fetchOrders(localClientOrderIdsInput: string[] = []): Promise<Order[]> {
  ...
  try {
    ...
    return list.filter((order: any) => sameIdentity(order, telegramId, username, localClientOrderIds)).map(normalizeOrder);
  } catch (error) {
